...
COLUMNS = ['c-modulation', 'c-delay', 'c-distortion', 'c-reverb', 'c-tremolo', 'c-dry', 'fx_class']
...
def main(parser):
    ...
    if args['append']:
        ...
        if 'c-dry' in df.columns and args['force']:
            ...
            # df = df.drop(columns=['c-modulation', 'c-delay', 'c-distortion', 'c-reverb', 'c-tremolo', 'c-dry', 'fx_class'])
        columns = list(df.columns) + ['c-modulation', 'c-delay', 'c-distortion', 'c-reverb', 'c-tremolo', 'c-dry', 'fx_class']
        df = df.reindex(columns=columns)
    else:
        df = pd.DataFrame(columns=['c-modulation', 'c-delay', 'c-distortion', 'c-reverb', 'c-tremolo', 'c-dry', 'fx_class'])
    ...
    for file in tqdm.tqdm(input_path.rglob('*.wav')):
        ...
        # print(conditioning)
        # print(df.loc[df["Unnamed: 0"] == file.stem, COLUMNS[:-1]])
        df.loc[df["Unnamed: 0"] == file.stem, COLUMNS[:-1]] = conditioning[0]
        df.loc[df["Unnamed: 0"] == file.stem, 'fx_class'] = fx_class
    df.to_csv(input_path / 'data_simple.csv')
    df.to_pickle(input_path / 'data_simple.pkl')
